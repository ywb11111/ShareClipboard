...
- Android 默认启用前台保活，通过常驻通知、CPU/Wi-Fi/组播锁，在界面关闭或从最近任务划走后继续保持设备心跳和接收
...
5. Android 10 及以上限制应用在后台读取剪贴板。手机端前台时可自动发送；在后台复制后，重新打开 ClipLink，连接建立时会自动补发当前内容，也可点击“立即同步当前剪贴板”。电脑到手机的接收会由前台服务持续处理。
6. 如果手动连接也失败，请把 ClipLink 加入系统防火墙的专用网络白名单，并确认路由器没有启用 AP 隔离。
7. Android 13 及以上建议允许通知权限，才能在通知栏看到保活状态；Android 17 及以上还需允许局域网访问权限。
...
- 协议 v3 与旧版只支持文本的客户端不兼容；建议手机与电脑同时升级到 1.4。

### 连接稳定性

- “设置 → 后台保持连接”仅在 Android 显示并默认开启。开启后会显示低优先级常驻通知，持有 CPU/Wi-Fi/组播锁，维持局域网发现、心跳和接收；可从通知中的“停止后台连接”主动关闭。
- 前台服务会增加耗电。若锁屏后仍被厂商省电策略中断，可在设置页打开系统电池优化设置，将 ClipLink 设为“不优化”或“不受限制”。系统“强行停止”、部分厂商的一键清理或重启后的自动启动限制无法由应用绕过。
- Android 仍禁止普通应用在后台自由读取本机剪贴板，因此后台复制的手机内容需要重新打开 ClipLink 后才能自动补发；电脑发来的内容仍可在后台接收。
